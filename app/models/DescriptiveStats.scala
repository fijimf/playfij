...
object DescriptiveStats {
  def apply(series: Series[_, Double]): DescriptiveStats = {
    (series.length, series.min, series.max) match {
      case (n, Some(min), Some(max)) => DescriptiveStats(n, series.mean, series.stdev, series.skew, series.kurt, min, series.percentile(0.25, PctMethod.NIST), series.median, series.percentile(0.75, PctMethod.NIST), max)
